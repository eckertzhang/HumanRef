...
def load_img(img_file, withmask=False):
    mask = None
    ...
    if not img_file.endswith("png"):
        ...
        if withmask and img.shape[-1]==4:
            img = cv2.cvtColor(img, cv2.COLOR_BGRA2RGBA)
            mask = img[:, :, 3:]
            mask[mask < 0.5 * 255] = 0
            mask[mask >= 0.5 * 255] = 1
            img = img[:,:,:3] * mask + 255*(1-mask).repeat(3, -1)
        else:
            img = cv2.cvtColor(img, cv2.COLOR_RGBA2BGR)
    name = os.path.basename(img_file)
    mask_file = os.path.join(os.path.dirname(img_file), 'mask', name)
    ...
    return torch.tensor(img).permute(2, 0, 1).unsqueeze(0).float(), img.shape[:2], mask
...
def mask_expanding(mask, k_size=9):
    """
...
def process_image(img_file, hps_type, single, input_res, detector, withmask=False):

    tgt_res = min(input_res * 2, 1024)
    img_raw, (in_height, in_width), mask = load_img(img_file, withmask=withmask)
    M_square = get_affine_matrix_wh(in_width, in_height, tgt_res, tgt_res)
    if mask is not None:
        mask = torch.tensor(mask*255.).permute(2, 0, 1).unsqueeze(0).float()
        img_raw_rgba = torch.cat([img_raw, mask], dim=1)
        ...
        mask_square = img_square[:,3:,:,:]
        mask_square[mask_square < 0.5 * 255] = torch.tensor(0.)
        mask_square[mask_square >= 0.5 * 255] = torch.tensor(1.)
        img_square = img_square[:,:3,:,:]
    else:
        img_square = warp_affine(
            img_raw,
            M_square[:, :2], (tgt_res, ) * 2,
    ...
    for idx in range(len(boxes)):
        ...
        if mask is not None:
            mask_square = mask_square[0].permute(1,2,0).numpy()
            mask_detection = mask_detection + (1-mask_square)
        ...
        img_mask = mask_expanding(torch.tensor(1-img_mask)[None], k_size=3)[0].numpy()
        img_mask = (1-img_mask)*1.
        img_crop[:,:,3:] = (img_mask*255.).astype(np.uint8)
        mean_icon = std_icon = (0.5, 0.5, 0.5)
        img_np = (img_rembg[..., :3] * img_mask).astype(np.uint8)
        img_icon = transform_to_tensor(input_res, mean_icon, std_icon)(
        ...
        if hps_type == 'pymafx':
            img_pymafx_lst.append(
                get_pymafx(
                    transform_to_tensor(input_res, constants.IMG_NORM_MEAN,
...
def process_image_nowarp(img_file, hps_type, single, input_res, detector, withmask=False):
    ...
    mask_square = img_raw_rgba[:,3:,:,:]
    img_square = img_raw_rgba[:,:3,:,:]
    mask_square[mask_square < 0.5 * 255] = torch.tensor(0.)
    mask_square[mask_square >= 0.5 * 255] = torch.tensor(1.)
    
    mask_square = mask_expanding(torch.tensor(1-mask_square.permute(0,2,3,1)), k_size=3)[0].numpy()
    mask_square = (1-mask_square)*1.
    img_square_rgba = torch.cat([img_square[0].permute(1,2,0), torch.tensor(mask_square*255.)], dim=-1)
    img_square_rgba = img_square_rgba.numpy().astype(np.uint8)
    # img_square_rgba[..., :3] = (img_square_rgba[..., :3] * mask_square).astype(np.uint8)
    mean_icon = std_icon = (0.5, 0.5, 0.5)
    img_np = (img_square_rgba[..., :3] * mask_square).astype(np.uint8)
    img_icon = transform_to_tensor(input_res, mean_icon, std_icon)(
        Image.fromarray(img_np)
    ) * torch.tensor(mask_square).permute(2, 0, 1)
    img_hps = transform_to_tensor(224, constants.IMG_NORM_MEAN,
                                    constants.IMG_NORM_STD)(Image.fromarray(img_np))
    ...
    img_crop_lst.append(torch.tensor(img_square_rgba).permute(2, 0, 1) / 255.0)
    img_icon_lst.append(img_icon)
    img_hps_lst.append(img_hps)
    img_mask_lst.append(torch.tensor(mask_square[..., 0]))
    landmark_lst.append(landmarks['body'])


    uncrop_param = {
        "ori_shape": [in_height, in_width], "box_shape": [input_res, input_res], 
        "square_shape": [in_height, in_width], #[tgt_res, tgt_res], 
        "M_square": torch.eye(3, dtype=M_square.dtype)[None], #M_square, 
        "M_crop": torch.eye(3, dtype=M_square.dtype)[None] #M_crop
    }

    # img_square_tgt = warp_affine(
    #     torch.cat([img_square[0].permute(1,2,0), torch.tensor(mask_square)], dim=-1)[None].permute(0,3,1,2),
    #     M_square[:, :2], (tgt_res, ) * 2,
    #     mode='bilinear',
    #     padding_mode='zeros',
    #     align_corners=True
    ...
    return_dict = {
        "img_icon": torch.stack(img_icon_lst).float(),    #[N, 3, res, res]
        "img_crop": torch.stack(img_crop_lst).float(),    #[N, 4, res, res]               
        "img_crop_1024": torch.stack(img_crop_lst).float(),    #[N, 4, res, res]               
        "img_hps": torch.stack(img_hps_lst).float(),    #[N, 3, res, res]
        "img_raw": img_raw,    #[1, 3, H, W]
        "img_mask": torch.stack(img_mask_lst).float(),    #[N, res, res]
        "uncrop_param": uncrop_param,
