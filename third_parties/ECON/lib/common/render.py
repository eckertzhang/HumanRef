...
class Render:
    ...
    def get_camera_batch(self, type="four", idx=None):
        ...
        # fov=torch.tensor(20.)
        # f=1/(2*torch.tan(torch.deg2rad(fov/2)))
        # cameras = PerspectiveCameras(
        #     device=self.device,
        #     R=R,
        #     T=T,
        #     focal_length=torch.tensor((f,f))[None],
        #     # focal_length=torch.tensor((intrinsic[0,0]/4.,intrinsic[1,1]/4.))[None],
        #     principal_point=torch.tensor((0,0))[None],  #shape (N, 2).
        #     # principal_point=torch.tensor((intrinsic[0,2],intrinsic[1,2]))[None],  #shape (N, 2).
        #     # K=intrinsic[None],   #shape (N, 4, 4)
        # )
