...
from lib.common.imutils import process_image, process_image_nowarp
...
class TestDataset:
    def __init__(self, cfg, device):
        ...
        self.withmask = cfg["withmask"]
        self.nowarp = cfg["nowarp"]
        self.dataset_type = cfg["dataset_type"]

        self.device = device

        folder_list = sorted(glob.glob(f"{self.image_dir}/*"))
        self.subject_list = [osp.join(item, "render/000.png") for item in folder_list]

        # self.name_list = [osp.splitext(osp.basename(xx))[0] for xx in self.subject_list]
        # smpl related
    ...
    def __getitem__(self, index):
        ...
        img_name = img_path.split("/")[-3] #.replace('-','_')

        arr_dict = process_image_nowarp(img_path, self.hps_type, self.single, 512, self.detector, withmask=self.withmask)
        arr_dict.update({"name": img_name})

        # Test: load SMPL in dataset
        datset_type = self.dataset_type
        arr_dict['datset_type'] = datset_type
        if datset_type == 'cape':
            smpl_path = osp.dirname(osp.dirname(img_path)).replace('cape_3views', 'cape/smpl')
            arr_dict['smpl_path'] = smpl_path
        elif datset_type == 'thuman2':
            smpl_path = '/apdcephfs/share_1330077/dataset/THuman2.0_Release/smplx_paras/'+img_name+'/mesh_smplx.obj'
            arr_dict['smpl_path'] = smpl_path
            arr_dict['smpl_param'] = '/apdcephfs/share_1330077/dataset/THuman2.0_Release/smplx_paras/'+img_name+'/smplx_param.pkl'
