...
class TestDataset:
    def __init__(self, cfg, device):
        ...
        if osp.isfile(self.image_dir) and (self.image_dir.endswith('.png') or self.image_dir.endswith('.jpg')):
            self.subject_list = [self.image_dir,]
        else:
    ...
    def __getitem__(self, index):
        ...
        arr_dict = process_image(img_path, self.hps_type, self.single, 512, self.detector, withmask=self.withmask)
        arr_dict.update({"name": img_name})

        arr_dict0 = process_image(img_path, self.hps_type, self.single, 1024, self.detector, withmask=self.withmask)
        arr_dict["img_crop_1024"] = arr_dict0['img_crop']
        del arr_dict0
