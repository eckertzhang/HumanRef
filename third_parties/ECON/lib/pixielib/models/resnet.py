...
def load_ResNet50Model0(weights=None):
    model = ResNet(Bottleneck, [3, 4, 6, 3])
    try:
        ww = torch.load(weights)
        copy_parameter_from_resnet(model, torchvision.models.resnet50(weights=ww).state_dict())
    except:
        copy_parameter_from_resnet(model, torchvision.models.resnet50(weights=models.ResNet50_Weights.DEFAULT).state_dict())
    return model
