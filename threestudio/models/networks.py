...
class ProgressiveBandHashGrid(nn.Module, Updateable):
    def __init__(self, in_channels, config, dtype=torch.float32):
        ...
        if 'zero_initial' in config and config['zero_initial']:
            self.encoding.params.data *= 0.
            self.encoding.register_parameter(name="params", param=self.encoding.params)
        self.n_output_dims = self.encoding.n_output_dims
...
def get_encoding(n_input_dims: int, config) -> nn.Module:
    ...
    elif config.otype == "HierHashGrid":
        encoding_config = config.copy()
        encoding_config["otype"] = "HashGrid"
        encoding = TCNNEncoding(n_input_dims, config_to_primitive(encoding_config))
    else:
...
class SDFNetwork(nn.Module):
    def __init__(self, dim_in: int, dim_out: int, config: dict):
        ...
        layers = [
            nn.Linear(dim_in, self.n_neurons),
        ]
        for i in range(self.n_hidden_layers - 1):
            if i+1 in self.skip_in:
                layers += [nn.Linear(self.n_neurons+dim_in, self.n_neurons),]
            else:
                layers += [nn.Linear(self.n_neurons, self.n_neurons),]
        layers += [nn.Linear(self.n_neurons, dim_out)]
        ...
        #         torch.nn.init.normal_(lin.weight, mean=torch.sqrt(torch.pi) / torch.sqrt(dims[l]), std=0.0001)
...
class MLP(nn.Module):
    def __init__(self, dim_in, dim_out, config: dict):
        super().__init__()
        self.dim_in = dim_in
        ...
        net = []
        for l in range(self.num_layers):
            net.append(nn.Linear(self.dim_in if l == 0 else self.dim_hidden, self.dim_out if l == self.num_layers - 1 else self.dim_hidden, bias=bias))
...
def get_mlp(n_input_dims, n_output_dims, config) -> nn.Module:
    ...
    elif config.otype == "SDFNetwork":
        network = SDFNetwork(n_input_dims, n_output_dims, config_to_primitive(config))
    elif config.otype == "MLPNetwork":
        network = MLP(n_input_dims, n_output_dims, config_to_primitive(config))
