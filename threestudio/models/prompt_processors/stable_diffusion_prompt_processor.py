...
class StableDiffusionPromptProcessor(PromptProcessor):
    ...
    def configure_text_encoder(self) -> None:
        self.tokenizer = AutoTokenizer.from_pretrained(
            os.path.join(os.getenv('WEIGHT_PATH'), self.cfg.pretrained_model_name_or_path), subfolder="tokenizer"
        )
        os.environ["TOKENIZERS_PARALLELISM"] = "false"
        self.text_encoder = CLIPTextModel.from_pretrained(
            os.path.join(os.getenv('WEIGHT_PATH'), self.cfg.pretrained_model_name_or_path), subfolder="text_encoder"
        ).to(self.device)
    ...
    def spawn_func(pretrained_model_name_or_path, prompts, cache_dir):
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            os.path.join(os.getenv('WEIGHT_PATH'), pretrained_model_name_or_path), subfolder="tokenizer"
        )
        text_encoder = CLIPTextModel.from_pretrained(
            os.path.join(os.getenv('WEIGHT_PATH'), pretrained_model_name_or_path),
