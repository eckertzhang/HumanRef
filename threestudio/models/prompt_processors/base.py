...
class PromptProcessorOutput:
    ...
    head_text_embeddings: Float[Tensor, "N Nf"]
    foot_text_embeddings: Float[Tensor, "N Nf"]
    text_embeddings_vd: Float[Tensor, "Nv N Nf"]
    uncond_text_embeddings_vd: Float[Tensor, "Nv N Nf"]
    head_text_embeddings_vd: Float[Tensor, "Nv N Nf"]
    foot_text_embeddings_vd: Float[Tensor, "Nv N Nf"]
    ...
    def get_local_text_embeddings(
        self,
        part_name, # support 'full' / 'head' / 'foot'
        elevation: Float[Tensor, "B"],
        azimuth: Float[Tensor, "B"],
        camera_distances: Float[Tensor, "B"],
        view_dependent_prompting: bool = True,
        ...
        if view_dependent_prompting:
            ...
            if part_name == 'full':
                ...
                text_embeddings = self.head_text_embeddings_vd[direction_idx]  # type: ignore
            elif part_name == 'foot':
                text_embeddings = self.foot_text_embeddings_vd[direction_idx]  # type: ignore
            uncond_text_embeddings = self.uncond_text_embeddings_vd[direction_idx]  # type: ignore
        else:
            if part_name == 'full':
                text_embeddings = self.text_embeddings.expand(batch_size, -1, -1)  # type: ignore
            elif part_name == 'head':
                text_embeddings = self.head_text_embeddings.expand(batch_size, -1, -1)  # type: ignore
            elif part_name == 'foot':
                text_embeddings = self.foot_text_embeddings.expand(batch_size, -1, -1)  # type: ignore
            uncond_text_embeddings = self.uncond_text_embeddings.expand(  # type: ignore
                batch_size, -1, -1
            )
    ...
    def get_local_text_embeddings_perp_neg(
        self,
        part_name, # support 'full' / 'head' / 'foot'
        elevation: Float[Tensor, "B"],
        azimuth: Float[Tensor, "B"],
        ...
        if part_name == 'full':
            ...
            side_emb = self.head_text_embeddings_vd[0]
            front_emb = self.head_text_embeddings_vd[1]
            back_emb = self.head_text_embeddings_vd[2]
            overhead_emb = self.head_text_embeddings_vd[3]
        elif part_name == 'foot':
            side_emb = self.foot_text_embeddings_vd[0]
            front_emb = self.foot_text_embeddings_vd[1]
            back_emb = self.foot_text_embeddings_vd[2]
            overhead_emb = self.foot_text_embeddings_vd[3]
...
class PromptProcessor(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        side_back_threshold: float = 90.0
    ...
    def configure(self) -> None:
        self._cache_dir =os.path.join(os.getenv('WEIGHT_PATH'), "threestudio_cache/text_embeddings")
        ...
        if self.cfg.view_dependent_prompt_front:
            self.directions = [
                ...
                DirectionConfig(
                    "side rear",
                    lambda s: f"side rear view of {s}",
                    lambda s: s,
                    lambda ele, azi, dis: (
                        shift_azimuth_deg(azi) <= 180 - self.cfg.back_threshold
                    ) & (shift_azimuth_deg(azi) > self.cfg.side_back_threshold)
                    | (shift_azimuth_deg(azi) >= -180 + self.cfg.back_threshold) & (shift_azimuth_deg(azi) < -self.cfg.side_back_threshold),
                ),
                DirectionConfig(
                    "front",
            ...
            self.directions = [
                ...
                DirectionConfig(
                    "side rear",
                    # lambda s: f"{s}, side rear view",
                    lambda s: f"{s}, side rear view, no face",
                    lambda s: s,
                    lambda ele, azi, dis: (
                        shift_azimuth_deg(azi) <= 180 - self.cfg.back_threshold
                    ) & (shift_azimuth_deg(azi) > self.cfg.side_back_threshold)
                ...
                DirectionConfig(
                    ...
                    lambda s: f"{s}, back view, no face",
                    lambda s: s,
                    lambda ele, azi, dis: (
                        shift_azimuth_deg(azi) > 180 - self.cfg.back_threshold
                    )
        ...
        self.head_prompt = "Head of " + self.prompt
        self.foot_prompt = "Lower legs and feet of " + self.prompt
        ...
        if self.cfg.use_prompt_debiasing:
            ...
            self.head_prompts_vd = [
                d.prompt(self.head_prompt) for d in self.directions
            ]
            self.foot_prompts_vd = [
                d.prompt(self.foot_prompt) for d in self.directions_nohead
            ]
    ...
    def load_text_embeddings(self):
        ...
        self.head_text_embeddings = self.load_from_cache(self.head_prompt)[None, ...]
        self.foot_text_embeddings = self.load_from_cache(self.foot_prompt)[None, ...]
        self.head_text_embeddings_vd = torch.stack(
            [self.load_from_cache(prompt) for prompt in self.head_prompts_vd], dim=0
        )
        self.foot_text_embeddings_vd = torch.stack(
            [self.load_from_cache(prompt) for prompt in self.foot_prompts_vd], dim=0
        )
    ...
    def __call__(self) -> PromptProcessorOutput:
        return PromptProcessorOutput(
            ...
            head_text_embeddings=self.head_text_embeddings,
            foot_text_embeddings=self.foot_text_embeddings,
            head_text_embeddings_vd=self.head_text_embeddings_vd,
            foot_text_embeddings_vd=self.foot_text_embeddings_vd,
