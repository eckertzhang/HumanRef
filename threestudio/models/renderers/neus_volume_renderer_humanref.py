...
@threestudio.register("neus-volume-renderer-humanref")
class NeuSVolumeRenderer(VolumeRenderer):
    ...
    class Config(VolumeRenderer.Config):
        ...
        return_normal_perturb: bool = True
    ...
    def forward(
        ...
        **kwargs
        ...
        def alpha_fn(t_starts, t_ends, ray_indices):
            ...
            inv_std = self.variance(sdf) #* 0.7  # Eckert
            if self.cfg.use_volsdf:
        ...
        # if self.training:
        #     rgb_fg_all.register_hook((lambda grad: print('*******', grad.max())))
        ...
        if stage == "coarse":
            ...
            comp_rgb_fg: Float[Tensor, "Nr Nc"] = nerfacc.accumulate_along_rays(
                weights[..., 0].detach(), values=rgb_fg_all, ray_indices=ray_indices, n_rays=n_rays
            )

        if 'ambient_shading' in kwargs and kwargs['ambient_shading'] != 'albedo':
            light_directions: Float[Tensor, "B ... 3"] = F.normalize(
                    t_light_positions - positions, dim=-1
                )
            ratio = kwargs['ambient_ratio']
            lambertian = ratio + (1 - ratio) * dot(geo_out['shading_normal'], light_directions).clamp(min=0.1)
            lambertian = nerfacc.accumulate_along_rays(weights[..., 0], values=lambertian, ray_indices=ray_indices, n_rays=n_rays)
            if kwargs['ambient_shading'] == 'textureless':
                comp_rgb_fg = lambertian.repeat(1, 3)
            elif kwargs['ambient_shading'] == 'lambertian':
                comp_rgb_fg = comp_rgb_fg * lambertian
        ...
        if stage == "coarse":
            ...
            depth = depth + 10. * (1.0 - opacity)
        else:
            comp_rgb = comp_rgb_fg + bg_color * (1.0 - opacity.detach())
            depth = depth + 10. * (1.0 - opacity.detach())
    ...
    def update_step(
        self, epoch: int, global_step: int, on_load_weights: bool = False, run_init: bool = False
    ) -> None:
        ...
        if self.cfg.grid_prune or run_init:

            def occ_eval_fn(x):
                sdf = self.geometry.forward_sdf(x)
                inv_std = self.variance(sdf) * 0.5  # Eckert
                if self.cfg.use_volsdf:
                    alpha = self.render_step_size * volsdf_density(sdf, inv_std)
                else:
                    estimated_next_sdf = sdf - self.render_step_size * 0.5
            ...
            if self.training and not on_load_weights:
                self.estimator.update_every_n_steps(
                    step=global_step, occ_eval_fn=occ_eval_fn, n=16,
                )
