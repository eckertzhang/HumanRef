...
def get_orthoghphic_ray_directions(
    H: int,
    W: int,
    radius: float,
    principal: Optional[Tuple[float, float]] = None,
    ...
    ## Here, directions can be seen as original points, while the correct direction should be [0,0,-1]
    directions: Float[Tensor, "H W 3"] = torch.stack(
        [(i - cx) * 2 / W * radius, -(j - cy) * 2 / H * radius, torch.zeros_like(i)], -1
    ) 
...
def get_orthoghphic_rays(
    origins: Float[Tensor, "... 3"],
    c2w: Float[Tensor, "... 4 4"],
    keepdim=False,
    noise_scale=0.0,
) -> Tuple[Float[Tensor, "... 3"], Float[Tensor, "... 3"]]:
    # Rotate ray origins & directions from camera coordinate to the world coordinate
    assert origins.shape[-1] == 3
    directions = torch.zeros_like(origins, device=origins.device)

    if origins.ndim == 2:  # (N_rays, 3)
        if c2w.ndim == 2:  # (4, 4)
            c2w = c2w[None, :, :]
        assert c2w.ndim == 3  # (N_rays, 4, 4) or (1, 4, 4)
        rays_o = (origins[:, None, :] * c2w[:, :3, :3]).sum(-1) 
        rays_o += c2w[:, :3, 3].expand(rays_o.shape)  # (N_rays, 3)
        directions[:, -1] = -1 #torch.ones_like(directions[:, -1])
        rays_d = (directions[:, None, :] * c2w[:, :3, :3]).sum(-1)  # (N_rays, 3)
    elif origins.ndim == 3:  # (H, W, 3)
        directions[:, :, -1] = -1
        assert c2w.ndim in [2, 3]
        if c2w.ndim == 2:  # (4, 4)
            ...
            rays_o += c2w[None, None, :3, 3].expand(rays_o.shape)  # (H, W, 3)
            rays_d = (directions[:, :, None, :] * c2w[None, None, :3, :3]).sum(
                -1
            )  # (H, W, 3)
        elif c2w.ndim == 3:  # (B, 4, 4)
            ...
            rays_o += c2w[:, None, None, :3, 3].expand(rays_o.shape)
            rays_d = (directions[None, :, :, None, :] * c2w[:, None, None, :3, :3]).sum(
                -1
            )  # (B, H, W, 3)
    elif origins.ndim == 4:  # (B, H, W, 3)
        directions[:, :, :, -1] = -1
        assert c2w.ndim == 3  # (B, 4, 4)
        rays_o = (origins[:, :, :, None, :] * c2w[:, None, None, :3, :3]).sum(
            -1
        ) 
        rays_o += c2w[:, None, None, :3, 3].expand(rays_o.shape)  # (B, H, W, 3)
        rays_d = (directions[:, :, :, None, :] * c2w[:, None, None, :3, :3]).sum(
            -1
        )  # (B, H, W, 3)
...
def get_projection_matrix(
    fovy: Float[Tensor, "B"], aspect_wh: float, near: float, far: float, device='cpu'
) -> Float[Tensor, "B 4 4"]:
    batch_size = fovy.shape[0]
    proj_mtx = torch.zeros(batch_size, 4, 4, dtype=torch.float32, device=device)
    proj_mtx[:, 0, 0] = 1.0 / (torch.tan(fovy / 2.0).to(device) * aspect_wh)
    proj_mtx[:, 1, 1] = -1.0 / torch.tan(
        fovy / 2.0
    ).to(device)  # add a negative sign here as the y axis is flipped in nvdiffrast output
