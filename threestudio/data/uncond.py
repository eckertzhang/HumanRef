...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if random.random() < 0.5:
            ...
            elevation_deg = (
                torch.rand(self.cfg.batch_size).to(self.device)
            ...
            elevation_range_percent = [
                (torch.tensor(self.cfg.elevation_range[0]).to(self.device) + 90.0) / 180.0,
                (torch.tensor(self.cfg.elevation_range[1]).to(self.device) + 90.0) / 180.0,
            ]
        ...
        if self.cfg.batch_uniform_azimuth:
            ...
            azimuth_deg = (
                torch.rand(self.cfg.batch_size).to(self.device) + torch.arange(self.cfg.batch_size).to(self.device)
            ) / self.cfg.batch_size * (
        ...
        camera_perturb: Float[Tensor, "B 3"] = (
            torch.rand(self.cfg.batch_size, 3).to(self.device) * 2 * self.cfg.camera_perturb
            - self.cfg.camera_perturb
        )
        camera_positions = camera_positions + camera_perturb
        # sample center perturbations from a normal distribution with mean 0 and std center_perturb
        center_perturb: Float[Tensor, "B 3"] = (
            torch.randn(self.cfg.batch_size, 3).to(self.device) * self.cfg.center_perturb
        )
        center = center + center_perturb
        # sample up perturbations from a normal distribution with mean 0 and std up_perturb
        up_perturb: Float[Tensor, "B 3"] = (
            torch.randn(self.cfg.batch_size, 3).to(self.device) * self.cfg.up_perturb
        )
        up = up + up_perturb
        ...
        if self.cfg.light_sample_strategy == "dreamfusion":
            ...
            light_direction: Float[Tensor, "B 3"] = F.normalize(
                camera_positions
                + torch.randn(self.cfg.batch_size, 3).to(self.device) * self.cfg.light_position_perturb,
                dim=-1,
            )
            ...
            light_azimuth = (
                torch.rand(self.cfg.batch_size).to(self.device) * math.pi - 2 * math.pi
            )  # [-pi, pi]
            light_elevation = (
                torch.rand(self.cfg.batch_size).to(self.device) * math.pi / 3 + math.pi / 6
            )  # [pi/6, pi/2]
        ...
        focal_length: Float[Tensor, "B"] = 0.5 * self.height / torch.tan(0.5 * fovy).to(self.device)
        directions: Float[Tensor, "B H W 3"] = self.directions_unit_focal[
            None, :, :, :
        ].to(self.device).repeat(self.cfg.batch_size, 1, 1, 1)

        # Importance note: the returned rays_d MUST be normalized!
        if self.cfg.use_orthograph:
            # directions[..., :2] *= camera_distances[0]
            rays_o, rays_d = get_orthoghphic_rays(directions, c2w, keepdim=True)
        else:
            directions[:, :, :, :2] = (
        ...
        proj_mtx: Float[Tensor, "B 4 4"] = get_projection_matrix(
            fovy, self.width / self.height, 0.1, 1000.0, device=self.device
...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        focal_length: Float[Tensor, "B"] = (
            0.5 * self.cfg.eval_height / torch.tan(0.5 * fovy).to(self.device)
        )
        if self.cfg.use_orthograph:
            directions_unit_focal =  get_orthoghphic_ray_directions(H=self.cfg.eval_height, W=self.cfg.eval_width, radius=self.cfg.radius).to(self.device)
            ...
            rays_o, rays_d = get_rays(directions, c2w, keepdim=True)
        proj_mtx: Float[Tensor, "B 4 4"] = get_projection_matrix(
            fovy, self.cfg.eval_width / self.cfg.eval_height, 0.1, 1000.0, device=self.device
        )  # FIXME: hard-coded near and far
        mvp_mtx: Float[Tensor, "B 4 4"] = get_mvp_matrix(c2w, proj_mtx)
